...
Fixes mypy type errors and eliminates `Any` usage in the minchoc project.
...
1. Fix all mypy errors reported by `uv run mypy minchoc`
...
Place new TypedDict classes in `minchoc/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`minchoc/typing.py` when shared.
...
1. Run `uv run mypy minchoc` and capture all errors.
...
4. Run `uv run mypy minchoc` again to verify fixes.
