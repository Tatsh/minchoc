...
If any changed files are under `minchoc/` or `tests/`, run the following agents **in order**:
...
- **changelog** - update `CHANGELOG.md` with entries for the changes. After it completes, check if
  ...
  the relevant commit. **Only run when changes affect users**: files under `minchoc/`, `tests/`,
...
For example, if a commit contains `minchoc/commands/main.py`,
`tests/test_main_command.py`, and `CHANGELOG.md`, the component is determined by
`minchoc/commands/main.py` and `tests/test_main_command.py` only.
...
- Changes span unrelated components (e.g. `minchoc/media.py` and `.claude/agents/release.md`).
...
For Python files, strip the `minchoc/` prefix and replace `/` with `.` (like module imports).

- Python file `minchoc/media.py` → `media:`.
- Multiple files under `minchoc/commands/` → `commands:`.
- Single command file `minchoc/commands/admin.py` → `commands.admin:`.
